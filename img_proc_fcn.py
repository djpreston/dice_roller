...
#    cv2.waitKey(0)
    ...
    img_bw = img_hsv[:,:,0]
    img_bw_sat = img_hsv[:,:,1]
    _,img_bw_hi = cv2.threshold(img_bw,240,255,cv2.THRESH_BINARY)
    _,img_bw_lo = cv2.threshold(img_bw,40,255,cv2.THRESH_BINARY)
    img_bw_lo = cv2.bitwise_not(img_bw_lo)
    img_bw = cv2.bitwise_or(img_bw_hi, img_bw_lo)
    _,img_bw_lo_sat = cv2.threshold(img_bw_sat,20,255,cv2.THRESH_BINARY)
#    img_bw_lo_sat = cv2.bitwise_not(img_bw_lo_sat)
    img_bw2 = cv2.bitwise_and(img_bw, img_bw_lo_sat)
    img_bw = cv2.bitwise_and(img_bw, img_bw_lo_sat)
    img_bin = img_bw
#    img_bin = cv2.bitwise_not(img_bw)
#    img_bw = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
#    img_no_bg = cv2.subtract(img_bw, background)
